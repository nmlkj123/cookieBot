...
async def on_message(message):
    if message.content.startswith('!서정'):
        lolm=['2019.07.18 [RIP]','(정보)그녀는 이세계 사람 입니다.','★뭐든지 잘하는 그녀는 대체; 어디있나요'
              ,'그녀는 군필여고생 후보 입니다. 당신도 똑같다구요? 어쩌라고요','너도울고 나도 우러따','분명 챌린저 였으나, 상대하는것은 마치 다이아와 같았으니 우리팀 피지컬에 혀를 내두르며 채팅을 치더라. \n저 우롱하는 손짓을 보라! 마치 캐리 한다는 듯이 욕을 던져대니 어이하여 죽지않으리오. 어허, 라인전에서 풀콤맞고 피가 너덜이 나니 그녀가 아이쿠 하며 길을 서두르더라.\n아아 억울하다. 내가 본것은 소녀이거늘, 어이하여 상대하는 것은 듬직한 남자같은가. 오늘도 그녀의 캐리로 눈을 적시니 아아 달이 밝구나. 언제서야 그녀가 음성채팅을 할까.\n오랜 친구 고문관 겨울은 그녀와 함께 게임을 끌며 풍월을 읊었건만 어인 일로 도플갱어가  있는가 하여 그녀에 물으니, 아아 그녀는 저 악마같은 웃음으로 디스코드에서 사라졌나이다. 채팅창을 보시옵소서 저 소녀의 탈을 쓴 마녀가 우리를 핍박하나이다.\n그녀 잡아 벌하고 싶으나 힘이 모자르구나. 하늘이 이를 어여삐 여겨 고소를 내리오니 서정아 서정아 어두운 뒷골목 에서 우리를를 해하지 말아라 해하지 말아라. 두려울 것은 그것이 아니거늘 다른 곳에서 화면넘어 웃고있는 그녀를 조심 하여라. 그녀는이미 이세계 사람이더라.\n2019.01.18 ']
    ...
    if message.content.startswith("!명령어"):
        ...
        embed.add_field(name='!재생 유튜브 링크', value='뮤직앱에 노래를 추가합니다.', inline=False)
        embed.add_field(name='!현재노래', value='현재 플레이중인 노래의 제목,정보를 봅니다. ', inline=False)
        embed.add_field(name='!재생목록', value='뮤직앱에 예약중인 플레이 리스트를 봅니다.', inline=False)
        embed.add_field(name='!스킵', value='현재곡 스킵 투표를 합니다.', inline=False)
